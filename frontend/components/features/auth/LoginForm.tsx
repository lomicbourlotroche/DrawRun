...
export default function LoginForm() {
  ...
  // ── Shared UI helpers ─────────────────────────────────────────────────────
  const inputClass =
    'w-full px-4 py-3.5 rounded-xl border-2 border-border bg-surface text-foreground text-base placeholder-muted outline-none transition-all focus:border-primary focus:ring-4 focus:ring-primary/10';
  ...
  const SubmitButton = ({ label }: { label: string }) => (
    <button
      type="submit"
      disabled={isLoading}
      className="w-full flex items-center justify-center gap-2 py-4 px-6 rounded-xl text-base font-bold text-foreground transition-all disabled:opacity-60 disabled:cursor-not-allowed hover:-translate-y-0.5 active:translate-y-0"
      style={{
        background: 'linear-gradient(135deg, var(--primary) 0%, var(--secondary) 100%)',
        boxShadow: '0 4px 14px color-mix(in srgb, var(--primary), transparent 65%)',
        minHeight: '52px',
      }}
    >
  ...
  const ErrorAlert = () =>
    error ? (
      <div
        role="alert"
        className="flex items-center gap-2.5 px-4 py-3 rounded-xl text-sm font-medium text-danger bg-danger/10 border border-danger/20"
      >
  ...
  const SuccessAlert = ({ message }: { message: string }) => (
    <div
      role="status"
      className="flex items-center gap-2.5 px-4 py-3 rounded-xl text-sm font-medium text-success bg-success/10 border border-success/20"
    >
      <svg className="w-4 h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
  ...
  return (
    <div className="min-h-screen w-full flex">
      ...
      <div className="hidden lg:flex lg:flex-1 relative overflow-hidden flex-col justify-between p-12"
        style={{
          background: 'linear-gradient(135deg, var(--neutral-900) 0%, var(--neutral-800) 60%, var(--neutral-900) 100%)',
        }}
      >
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute bottom-0 right-0 w-[400px] h-[400px] rounded-full opacity-10"
            style={{ background: 'radial-gradient(circle, var(--secondary) 0%, transparent 70%)', transform: 'translate(30%, 30%)' }} />
          {/* Grid pattern */}
          <div className="absolute inset-0 opacity-[0.04]"
            style={{ backgroundImage: 'linear-gradient(var(--surface) 1px, transparent 1px), linear-gradient(90deg, var(--surface) 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
        </div>
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-semibold"
            style={{ background: 'color-mix(in srgb, var(--primary), transparent 85%)', borderColor: 'color-mix(in srgb, var(--primary), transparent 70%)', color: 'var(--primary)' }}>
            <span className="w-2 h-2 rounded-full bg-primary animate-pulse" />
            v3.42 Elite • ScienceEngine™
          </div>

          <div>
            <h1 className="text-5xl xl:text-6xl font-extrabold text-foreground leading-[1.1] tracking-tight">
              DOMINEZ VOS
              <br />
            ...
            <p className="mt-4 text-lg text-muted leading-relaxed max-w-sm">
              L&apos;application de performance ultime intégrant le moteur{' '}
              <span className="text-foreground font-semibold">Jack Daniels VDOT V6.4</span>{' '}
          ...
          <div className="grid grid-cols-1 gap-3">
            {features.map(({ icon: Icon, label, sub }) => (
              <div key={label}
                ...
                style={{ background: 'color-mix(in srgb, var(--surface), transparent 97%)', borderColor: 'color-mix(in srgb, var(--surface), transparent 93%)' }}>
        ...
        <div className="relative z-10 flex items-center gap-8 pt-8 border-t"
          style={{ borderColor: 'color-mix(in srgb, var(--surface), transparent 92%)' }}>
          {[
            ...
            <div key={s.label}>
              ...
              <p className="text-xs text-muted uppercase tracking-wider mt-0.5">{s.label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* ═══════════════════════════════════════════════
          RIGHT PANEL — form
          ═══════════════════════════════════════════════ */}
      <div className="flex-1 lg:max-w-[480px] flex flex-col bg-background relative">

        {/* Back link */}
        <div className="absolute top-5 left-5 z-10">
          <Link href="/"
            className="inline-flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium text-muted hover:text-foreground hover:bg-surface border border-transparent hover:border-border transition-all">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <button
                          ...
                          className="absolute right-3.5 top-1/2 -translate-y-1/2 text-muted hover:text-foreground transition-colors p-2 min-h-[44px] min-w-[44px]"
              ...
              <>
                ...
                <div className="flex items-center justify-center gap-5 mt-8 pt-6 border-t border-border">
                  {['🔒 Sécurisé SSL', '🇫🇷 Hébergé en France', '✓ Gratuit'].map((badge) => (
                    <span key={badge} className="text-xs text-muted font-medium">{badge}</span>
                  ))}
                </div>
              </>
            )}
