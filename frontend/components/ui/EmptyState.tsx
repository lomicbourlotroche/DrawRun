    ...
    >
      ...
      >
        {React.cloneElement(displayIcon as React.ReactElement, {
          className: cn('text-primary-600 dark:text-primary-400', compact ? 'w-6 h-6' : 'w-8 h-8 md:w-10 md:h-10'),
        })}
      </div>

      <h3 className={cn('font-bold text-foreground', compact ? 'text-lg' : 'text-xl md:text-2xl')}>{displayTitle}</h3>

      {displayDescription && (
        <p className={cn('mt-3 text-muted', compact ? 'text-sm' : 'text-sm md:text-base')}>{displayDescription}</p>
      )}

      {action && <div className={cn('mt-6', compact ? 'mt-4' : 'mt-6')}>{action}</div>}
    </div>
  );
}
...
export function EmptyStateWithIllustration({
  ...
  return (
    ...
    >
      <div className="mb-8 text-primary-200 dark:text-primary-800/30">{illustration}</div>

      <h3 className="text-xl md:text-2xl font-bold text-foreground">{title}</h3>

      <p className="mt-3 text-muted text-sm md:text-base max-w-md">{description}</p>
...
export function EmptyList({
  ...
  return (
    <div role="region" aria-label={title} className={cn('flex flex-col items-center justify-center py-12', className)}>
...
export function EmptyTable({
  ...
  return (
    <div role="region" aria-label={title} className={cn('w-full', className)}>
      <div
        className={cn('flex flex-col items-center justify-center py-12', 'bg-surface border border-border rounded-xl')}
      >
