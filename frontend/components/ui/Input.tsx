...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, label, error, hint, leftIcon, rightIcon, id, clearable = false, ...props }, ref) => {
    ...
    const fallbackId = label ? label.toLowerCase().replace(/\s+/g, '-') : `input-${uniqueId}`;
    const inputId = id || fallbackId;
    const errorId = error ? `error-${inputId}-${uniqueId}` : undefined;
    const hintId = hint ? `hint-${inputId}-${uniqueId}` : undefined;
