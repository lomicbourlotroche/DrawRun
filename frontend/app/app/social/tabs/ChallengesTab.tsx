                  ...
                  <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted">
                            {mode.icon} {mode.label} · {getTypeInfo(c.type).icon} {c.target_value} {c.target_unit}
                          </p>
                        </div>
                      </div>
                      <span
                    ...
                    {c.challenge_mode === 'streak' && (
                      <div className="flex items-center gap-4 text-sm">
                        <div className="flex items-center gap-1">
                          <span className="text-peak/80">🔥</span>
                          <span className="font-bold">{c.streak_current || 0}</span>
                          <span className="text-muted text-xs">actuel</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <span className="text-warning/80">⭐</span>
                          <span className="font-bold">{c.streak_best || 0}</span>
                    ...
                    <div className="space-y-1">
                      <div className="relative w-full bg-border rounded-full h-3">
                        <div
                          className={`h-3 rounded-full transition-all duration-500 ${getProgressColor(pct)}`}
                          style={{ width: `${pct}%` }}
                        />
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {!alreadyJoined ? (
                      <Button
                        ...
                        aria-label={`Rejoindre le défi ${c.title}`}
                      >
                        Rejoindre
                      </Button>
                    ) : (
                      <span className="text-xs px-2 py-1 rounded-full bg-success/10 text-success font-medium shrink-0">
                        ✓ Rejoint
                      </span>
                    )}
                  </div>
