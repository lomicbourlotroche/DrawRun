      ...
      <div className="bg-card rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
        <div className="px-6 pt-5 pb-4 border-b border-border">
          ...
          <p className="text-xs text-muted mt-2">
            {step === 1 && (showPresets ? 'Étape 1 — Choisir le mode' : 'Étape 1 — Mode du défi')}
            {step === 2 && "Étape 2 — Définir l'objectif"}
            {step === 3 && 'Étape 3 — Personnaliser'}
          </p>
        </div>
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Décrivez votre défi..." rows={3} className="w-full px-3 py-2 rounded-xl bg-background border border-border text-sm focus:border-primary outline-none resize-none" />
              </div>
              <div>
        ...
        <div className="px-6 py-4 border-t border-border flex gap-3">
          {step > 1 && <Button variant="secondary" onClick={() => setStep(s => s - 1)} className="rounded-xl">← Retour</Button>}
          {step < 3 ? (
            <Button onClick={() => setStep(s => s + 1)} className="flex-1 rounded-xl" disabled={step === 2 && !form.target_value && form.challenge_mode !== 'frequency'}>Suivant →</Button>
          ) : (
