...
export function GpxProfileBadge({ profile }: { profile: GpxProfile }) {
  ...
  const terrainColor = {
    ...
    rolling: 'bg-warning-50 text-warning-700',
    mountainous: 'bg-danger-50 text-danger-700',
  }[profile.terrainType];
  return (
    <div className="p-3 rounded-lg bg-primary/5 border border-primary/20 space-y-2">
      ...
      <div className="grid grid-cols-3 gap-2 text-xs">
        <div className="text-center">
          <p className="font-bold text-success">+{profile.elevGain}m</p>
          <p className="text-muted">Dénivelé +</p>
        </div>
        <div className="text-center">
          <p className="font-bold text-danger">-{profile.elevLoss}m</p>
          <p className="text-muted">Dénivelé -</p>
        </div>
        <div className="text-center">
          <p className="font-bold">{profile.gainPerKm}m/km</p>
          <p className="text-muted">Gain/km</p>
        </div>
      </div>
    </div>
  );
