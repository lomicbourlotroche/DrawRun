    ...
    <Card variant="glass">
      ...
      <CardContent>
        <p className="text-sm text-muted mb-6">
          L&apos;Efficiency Factor (EF) mesure votre vitesse ajustée à la pente (GAP) par rapport à votre fréquence
          cardiaque moyenne. Une hausse de l&apos;EF sur le long terme indique une amélioration de votre condition
          aérobie.
        </p>
          ...
          {efData.length >= 2 ? (
            <div className="w-full h-full flex flex-col justify-end gap-1">
              ...
              <div className="flex justify-between text-[10px] text-muted pt-1">
                <span>
                  {efData.length > 0
                    ? new Date(efData[0].date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })
                    : ''}
                </span>
                <span>
                  {efData.length > 0
                    ? new Date(efData[efData.length - 1].date).toLocaleDateString(undefined, {
                        month: 'short',
                        day: 'numeric',
                      })
                    : ''}
                ...
                {trailActivities.slice(0, 6).map((a) => (
                  <tr key={a.id} className="border-b border-border/50 last:border-0 hover:bg-muted/5 transition-colors">
                    <td className="py-2.5 text-muted">
                      {new Date(a.date || a.start_date || '').toLocaleDateString(undefined, {
                        day: 'numeric',
                        month: 'short',
                      })}
                    </td>
