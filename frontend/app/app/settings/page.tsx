...
export default function SettingsPage() {
  ...
  const loadPreferences = useCallback(async () => {
    ...
    try {
      ...
      if (prefs.theme) {
        const t = prefs.theme as 'light' | 'dark' | 'auto';
        setThemeMode(t);
        if (t !== 'auto') setMode(t);
        else {
      ...
      if ((prefs as Record<string, unknown>).language)
        setLanguage((prefs as Record<string, unknown>).language as 'fr' | 'en');
      if ((prefs as Record<string, unknown>).density)
        setDensity((prefs as Record<string, unknown>).density as 'compact' | 'normal' | 'comfortable');
    } catch {}
  }, [setMode, setLanguage]);
  ...
  useEffect(() => {
    api
      ...
      .then((s) =>
        setSyncStatus(
          s as unknown as { lastSync?: string; provider?: string; status?: string; garmin_status?: string },
        ),
      )
      .catch(() => {});
  }, []);
  ...
  const enablePushNotifications = async () => {
    if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
      toast.error('Non support\u00e9 par votre navigateur');
      return;
    }
  ...
  const handleFullResync = async () => {
    ...
    try {
      ...
      setSyncStatus(
        status as unknown as { lastSync?: string; provider?: string; status?: string; garmin_status?: string },
      );
    } catch {
  ...
  const lastSyncDate =
    syncStatus?.lastSync || ((syncStatus as Record<string, unknown>)?.garmin_last_sync as string | undefined);
  ...
  return (
    <div className="animate-fade-in max-w-4xl mx-auto space-y-5 pb-8">
      ...
      <Card variant="glass" accent="primary" className="animate-slide-up delay-200">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <RefreshCw className={`w-5 h-5 ${isSyncingGarmin ? 'animate-spin text-primary' : 'text-primary'}`} />
            Synchronisation
          </CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span
                    className={`w-2 h-2 rounded-full ${isSyncingGarmin ? 'bg-primary animate-pulse' : syncStatus?.garmin_status === 'error' ? 'bg-danger' : 'bg-success'}`}
                  />
        ...
        <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                },
                {
                  ...
                  Icon: Monitor,
                  ic: 'text-muted',
                  pb: 'bg-gradient-to-br from-white to-surface',
                  pbd: 'border-border',
                },
              ].map((t) => (
                <button
                  ...
                  className={`relative p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 ${themeMode === t.id ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/40'}`}
                >
                  {themeMode === t.id && (
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { id: 'metric' as const, label: 'M\u00e9trique', sub: 'km \u00b7 kg \u00b7 m' },
                { id: 'imperial' as const, label: 'Imp\u00e9rial', sub: 'mi \u00b7 lbs \u00b7 ft' },
              ].map((u) => (
                <button
                  key={u.id}
                  onClick={() => handleUnitsChange(u.id)}
                  className={`p-4 rounded-xl border-2 transition-all text-left ${units === u.id ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/40'}`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    {units === u.id && <Check className="w-4 h-4 text-primary" />}
                  </div>
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { id: 'compact' as const, label: 'Compact', sub: "Plus d'infos" },
                { id: 'normal' as const, label: 'Normal', sub: '\u00c9quilibr\u00e9' },
                { id: 'comfortable' as const, label: 'A\u00e9r\u00e9', sub: "Plus d'espace" },
              ].map((d) => (
                <button
                  key={d.id}
                  onClick={() => handleDensityChange(d.id)}
                  className={`p-3 rounded-xl border-2 transition-all text-center ${density === d.id ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/40'}`}
                >
      ...
      <Card variant="glass" accent="info" className="animate-slide-up delay-400">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Globe className="w-5 h-5 text-peak" />
            Langue
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 gap-3">
            {[
              { id: 'fr', label: 'Fran\u00e7ais', flag: <Flag className="w-5 h-5 text-primary" /> },
              { id: 'en', label: 'English', flag: <Globe className="w-5 h-5 text-muted" /> },
            ].map((l) => (
              <button
                key={l.id}
                onClick={() => handleLanguageChange(l.id)}
                className={`p-4 rounded-xl border-2 transition-all text-left ${language === l.id ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/40'}`}
              >
                <div className="flex items-center justify-between">
                  ...
                  {language === l.id && <Check className="w-4 h-4 text-primary" />}
                </div>
              </button>
            ))}
          </div>
      ...
      <Card variant="glass" accent="primary" className="animate-slide-up delay-500">
        ...
        <CardContent>
          <div className="flex items-center justify-between p-4 rounded-xl bg-surface border border-border">
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-xl ${pushEnabled ? 'bg-success/10' : 'bg-border'}`}>
              ...
              <div>
                ...
                <p className="text-xs text-muted">
                  {pushEnabled ? 'Actives sur cet appareil' : 'D\u00e9sactiv\u00e9es'}
                </p>
              </div>
            </div>
            <button
              ...
              className={`w-12 h-6 rounded-full transition-all relative disabled:opacity-50 ${pushEnabled ? 'bg-success' : 'bg-border'}`}
            >
              <div
                className={`absolute top-1 w-4 h-4 bg-surface rounded-full shadow transition-all ${pushEnabled ? 'left-7' : 'left-1'}`}
              />
            </button>
          </div>
          <p className="text-xs text-muted mt-3">
            Recevez des alertes en temps r\u00e9el pour les demandes d&apos;ami, draws et commentaires.
          </p>
        </CardContent>
      </Card>
      ...
      <Card variant="glass" accent="primary" className="animate-slide-up delay-600">
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between p-4 rounded-xl bg-surface border border-border">
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-xl ${twoFAEnabled ? 'bg-success/10' : 'bg-border'}`}>
                <Shield className={`w-5 h-5 ${twoFAEnabled ? 'text-success' : 'text-muted'}`} />
              </div>
              <div>
                <p className="font-medium text-sm text-foreground">Authentification 2 facteurs</p>
                <p className="text-xs text-muted">{twoFAEnabled ? '\u2713 Activ\u00e9e' : 'D\u00e9sactiv\u00e9e'}</p>
              </div>
            </div>
            {twoFAEnabled ? (
              <Button
          ...
          <button
            ...
            className="w-full flex items-center gap-3 p-4 rounded-xl bg-surface border border-border hover:border-primary/40 transition-all text-left"
          >
            <div className="p-2 rounded-xl bg-border">
              <Lock className="w-5 h-5 text-muted" />
            </div>
            <div className="flex-1">
              <p className="font-medium text-sm text-foreground">Changer le mot de passe</p>
              <p className="text-xs text-muted">Modifier votre mot de passe de connexion</p>
            </div>
      ...
      <Card variant="glass" accent="danger" className="animate-slide-up delay-700">
        ...
        <CardContent className="space-y-3">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 p-4 rounded-xl bg-danger/5 border border-danger/20 hover:border-danger/40 transition-all text-left"
          >
            <div className="p-2 rounded-xl bg-danger/10">
              <Trash2 className="w-5 h-5 text-danger" />
            </div>
            <div>
              <p className="font-medium text-sm text-danger">Supprimer le compte</p>
            ...
            <>
              ...
              {twoFAQrUri && (
                <div className="p-4 bg-surface rounded-xl text-center">
                  <img
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(twoFAQrUri)}`}
                    alt="QR 2FA"
              ...
              <div className="p-3 rounded-xl bg-surface border border-border">
                <p className="text-xs text-muted mb-1">Cl\u00e9 secr\u00e8te</p>
                <p className="text-xs font-mono text-foreground break-all">{twoFASecret}</p>
              </div>
              <Input
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Supprimer le compte" size="sm">
        <div className="space-y-4">
          <div className="flex items-start gap-3 p-4 rounded-xl bg-danger/10 border border-danger/30">
            ...
            <div>
              <p className="text-sm font-medium text-danger">Action irr\u00e9versible</p>
