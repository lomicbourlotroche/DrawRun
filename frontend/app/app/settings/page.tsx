...
export default function SettingsPage() {
  ...
  const loadPreferences = useCallback(async () => {
    try { const profile = await api.getProfile(); setTwoFAEnabled(!!(profile as UserType).twofa_enabled); } catch { // ignore
}
    try {
  ...
  useEffect(() => {
    const check = async () => {
      ...
      try { const reg = await navigator.serviceWorker.ready; const sub = await reg.pushManager.getSubscription(); setPushEnabled(!!sub); setPushSubscription(sub); } catch { // ignore
}
    };
    check();
  }, []);
  ...
  useEffect(() => {
    api.getSyncStatus().then((s) => setSyncStatus(s as unknown as { lastSync?: string; provider?: string; status?: string; garmin_status?: string })).catch(() => { // ignore
});
  }, []);
  ...
  const handleThemeChange = (newTheme: 'light' | 'dark' | 'auto') => {
    ...
    api.updatePreferences({ theme: newTheme }).catch(() => { // ignore
});
  };
  const handleLanguageChange = (newLanguage: string) => {
    setLanguage(newLanguage as 'fr' | 'en');
    api.updatePreferences({ language: newLanguage } as Parameters<typeof api.updatePreferences>[0]).catch(() => { // ignore
});
  };
  const handleUnitsChange = (newUnits: 'metric' | 'imperial') => {
    setUnits(newUnits); api.updatePreferences({ units: newUnits }).catch(() => { // ignore
});
  };
  const handleDensityChange = (newDensity: 'compact' | 'normal' | 'comfortable') => {
    ...
    api.updatePreferences({ density: newDensity } as Parameters<typeof api.updatePreferences>[0]).catch(() => { // ignore
});
  };
