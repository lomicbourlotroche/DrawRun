...
const features = [
      ...
      <div className="bg-gradient-to-br from-primary/5 to-primary/10 rounded-xl p-4 border border-primary/20">
        <div className="space-y-2">
          <div className="flex justify-between text-xs font-medium">
            <span className="text-primary">VDOT</span>
            <span className="text-foreground">52.5</span>
          </div>
          <div className="w-full h-16 bg-gradient-to-r from-primary/20 to-primary/30 rounded-lg relative overflow-hidden">
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 40">
              <path d="M0,30 Q25,10 50,20 T100,15" stroke="var(--primary)" strokeWidth="2" fill="none" />
            </svg>
          </div>
        </div>
      </div>
    ...
    mockup: (
      <div className="bg-gradient-to-br from-success/5 to-success/10 rounded-xl p-4 border border-success/20">
        ...
        <div className="mt-2 space-y-1">
          ...
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-peak" />
    ...
    mockup: (
      <div className="bg-gradient-to-br from-warning/5 to-warning/10 rounded-xl p-4 border border-warning/20">
        <div className="text-center">
          <div className="text-2xl font-bold text-warning">42.2 km</div>
          ...
          <svg className="w-full h-8 mt-1" viewBox="0 0 100 20">
            <polyline points="0,15 20,12 40,14 60,10 80,11 100,8" stroke="var(--warning)" strokeWidth="2" fill="none" />
          </svg>
        </div>
      </div>
    ),
  ...
  },
  {
    ...
    color: 'peak',
    mockup: (
      <div className="bg-gradient-to-br from-peak/5 to-peak/10 rounded-xl p-4 border border-peak/20">
        <div className="flex justify-between text-xs mb-2">
          <span className="text-peak">CTL</span>
          <span className="text-peak">ATL</span>
          <span className="text-peak">TSB</span>
        </div>
        <div className="w-full h-16 relative">
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 64">
            <path d="M0,50 Q25,40 50,45 T100,42" stroke="var(--primary)" strokeWidth="1.5" fill="none" opacity="0.8" />
            <path d="M0,55 Q25,50 50,52 T100,48" stroke="var(--danger)" strokeWidth="1.5" fill="none" opacity="0.8" />
          </svg>
        </div>
        <div className="text-right text-xs text-peak font-medium mt-1">+25</div>
      </div>
    ),
    link: '/guides/pmc',
  },
  {
    ...
    color: 'danger',
    mockup: (
      <div className="bg-gradient-to-br from-danger/5 to-danger/10 rounded-xl p-4 border border-danger/20">
  ...
  },
  {
    ...
    color: 'recovery',
    mockup: (
      <div className="bg-gradient-to-br from-recovery/5 to-recovery/10 rounded-xl p-4 border border-recovery/20">
        <div className="flex -space-x-2 mb-2">
          {['A', 'B', 'C', 'D'].map((letter, i) => (
            <div
              key={letter}
              className="w-6 h-6 rounded-full bg-primary/20 text-primary text-xs flex items-center justify-center font-semibold border-2 border-primary"
              style={{ zIndex: 4 - i }}
            >
              {letter}
            </div>
              ...
              >
                <div className="flex flex-col h-full">
                  ...
                  <Button
                    ...
                    className="w-fit self-start text-primary hover:text-primary/80"
                  >
                    En savoir plus →
                  </Button>
                </div>
        ...
        <div className={`mt-16 transition-all duration-700 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <Card variant="glass" padding="lg" className="max-w-2xl mx-auto">
            <div className="grid grid-cols-3 gap-6 text-center">
              ...
              <div className="p-4">
                <div className="w-12 h-12 mx-auto mb-3 bg-peak/10 rounded-xl flex items-center justify-center">
                  <Heart className="w-6 h-6 text-peak" />
                </div>
