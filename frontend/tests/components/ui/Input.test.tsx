...
describe('Input component', () => {
  ...
  it('sets aria-invalid to false when no error is provided', () => {
    render(<Input />);
    const input = screen.getByRole('textbox');
    expect(input).toHaveAttribute('aria-invalid', 'false');
  });

  it('sets aria-describedby to link to the error message', () => {
    render(<Input error="Error message" id="test-input" />);
    const input = screen.getByRole('textbox');
    const errorText = screen.getByText('Error message');

    // The <p> wrapping the error text is what we give the ID to
    // We expect the ID of the error wrapper to match the aria-describedby attribute
    expect(input).toHaveAttribute('aria-describedby', 'test-input-error');
  });

  it('sets aria-describedby to link to the hint message', () => {
    render(<Input hint="Hint message" id="test-input" />);
    const input = screen.getByRole('textbox');
