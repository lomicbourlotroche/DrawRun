...
describe('Input component', () => {
  ...
  it('associates error message with input using aria-describedby', () => {
    ...
    expect(errorMessage).toHaveAttribute('id', input.getAttribute('aria-describedby') as string);
  });

  it('associates hint message with input using aria-describedby', () => {
    render(<Input hint="Hint message" />);
    const input = screen.getByRole('textbox');
    const hintMessage = screen.getByText('Hint message');

    expect(input).toHaveAttribute('aria-invalid', 'false');
    expect(input).toHaveAttribute('aria-describedby', expect.stringMatching(/.*-hint/));
    expect(hintMessage).toHaveAttribute('id', input.getAttribute('aria-describedby') as string);
  });
});
