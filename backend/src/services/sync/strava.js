...
async function performSync(userId) {
    ...
    try {
        ...
        const stravaEmail = user.strava_client_id;
        const stravaPassword = decrypt(user.strava_client_secret) || user.strava_client_secret;
        ...
        if (hasCookies) {
            ...
            if (!valid) {
                ...
                await loginToStrava(page, stravaEmail, stravaPassword);
