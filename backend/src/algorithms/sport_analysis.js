...
const SportAnalysis = {
    ...
    _calcTSS: (act, constants, profile) => {
        ...
        const { durationHours, avgHR, avgPower, elevation, rpe } = act;
    ...
    _calcHRZones: (act, profile) => {
        ...
        if (hrPercent >= 0.90) currentZone = 5;

        else if (hrPercent >= 0.80) currentZone = 4;

        else if (hrPercent >= 0.70) currentZone = 3;

        else if (hrPercent >= 0.60) currentZone = 2;
    ...
    _analyzeRun: (activity, profile, streams) => {
        ...
        if (vdot && pace?.speedKmh > 0) {
            ...
            const vo2 = vdot * (0.6 + (hrZones?.percent || 70) / 100 * 0.3);
